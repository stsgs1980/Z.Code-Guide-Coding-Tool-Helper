    ...
    <section id="safety-confirm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className={`p-4 rounded-xl ${th("nyc-card-enhanced", "border border-oklch(0.88 0 0) bg-oklch(0.98 0 0)")}`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="font-semibold text-sm">{data.subsections.safetyConfirm.title}</h4>
          </div>
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-semibold text-sm">{data.subsections.decisionOptions.title}</h4>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {data.subsections.decisionOptions.options.map((opt) => (
            ...
            >
              ...
              <p className={`text-xs mt-2 mb-1 ${th("text-white/70", "text-oklch(0.35 0 0)")}`}>{opt.desc}</p>
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${th("bg-white/5", "bg-oklch(0.93 0 0)")}`}>
            <AlertTriangle className="h-4 w-4 text-nyc-taxi" />
          </div>
          <h4 className="font-semibold text-sm">{data.subsections.scenarios.title}</h4>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <Icon className="h-3 w-3 text-nyc-taxi" />
                  <span className="font-medium text-xs">{item.title}</span>
                </div>
                <p className={`text-[10px] ${th("text-white/50", "text-oklch(0.50 0 0)")}`}>{item.desc}</p>
              </div>
            );
          })}
        </div>
        ...
        {data.subsections.scenarios.image && (
          <ClickableImage
            ...
            alt="Safety Confirm"
            className={`w-full max-w-md rounded-lg border ${th("border-white/10", "border-oklch(0.88 0 0)")}`}
            thumbnailHeight={100}
          />
        )}
      </motion.div>
