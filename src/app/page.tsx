...
import { Menu, X, ArrowLeft, Monitor, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {currentPage === "guide" && (
        <>
          ...
          {/* Content sections */}
          <main className="md:ml-14 md:pr-8 w-full px-4 sm:px-6 pb-20 flex-1 break-words">
            <QuickStartSection />
            <HelperSection />

            {/* ZCode Desktop — promo card linking to separate page */}
            <motion.div
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-bold nyc-gradient-text">ZCode Desktop</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${th('bg-nyc-taxi/10 text-nyc-taxi', 'bg-oklch(0.78 0.16 85 / 15%) text-oklch(0.78 0.16 85)')}`}>ADE</span>
                      </div>
                      <p className={`text-sm ${th('text-white/50', 'text-oklch(0.45 0 0)')}`}>
                        Agentic Development Environment — полноценная среда AI-разработки с мультиагентной работой, терминалом, предпросмотром и безопасностью
                      </p>
                    </div>
                  </div>
                  <ChevronRight className={`h-5 w-5 flex-shrink-0 ml-4 group-hover:translate-x-1 transition-transform ${th('text-nyc-taxi/50 group-hover:text-nyc-taxi', 'text-oklch(0.78 0.16 85 / 40%) group-hover:text-oklch(0.78 0.16 85)')}`} />
                </div>
              </button>
            </motion.div>
      ...
      {currentPage === "zcode" && (
        <main className="md:ml-14 md:pr-8 w-full px-4 sm:px-6 pb-20 flex-1 pt-14 md:pt-0 break-words">
